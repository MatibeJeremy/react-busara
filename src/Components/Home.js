import './styles.css';
import {useEffect, useState} from "react";
import {useHistory} from "react-router-dom";
import axios from "axios";
function Home(){
    let history = useHistory();
    const [user,setUser] = useState([]);

    useEffect( ()=>{
        const url = "http://fullstack-role.busara.io/api/v1/users/current-user";

        axios.get(url,{
            headers:{
                'Authorization': `Bearer ${localStorage.getItem('access_token')}`
            }
        })
            .then((response) => {
                console.log(response.data);
                setUser(response.data);
                console.log(user);
            }).catch((error)=>{
            console.log(error);
        });
    },[]);

    useEffect(()=>{
        console.log(user);
    })
    return (
        <div>
            <div className="card" id="card">
                <svg className="card-img-top" xmlns="http://www.w3.org/2000/svg" width="150" height="150" fill="currentColor"
                     className="bi bi-person" viewBox="0 0 16 16">
                    <path
                        d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0zm4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10z"/>
                </svg>
                    <div className="card-body">
                        <h5 className="card-title">User Details</h5>
                        <p className="card-text">Name: {user.name || 'Loading...'}</p>
                        <p className="card-text">Email: {user.email || 'Loading...'}</p>
                        <p className="card-text">Phone Number: {user.phone_number || 'Loading...'}</p>
                        <button onClick={() => {history.push('/questions')}} className="btn btn-primary">Questions</button>
                    </div>
            </div>
        </div>
    )
}
export default Home;